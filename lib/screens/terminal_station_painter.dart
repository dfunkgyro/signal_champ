...
class TerminalStationPainter extends CustomPainter with CollisionVisualEffects {
  ...
  void _paintLayerComponents(Canvas canvas, layer) {
    ...
    _drawReservationPreview(canvas);
  ...
  void _drawSignal(Canvas canvas, Signal signal) {
    ...
    final polePaint = Paint()
      ..color = signal.commissioned
          ? themeData.signalPoleColor
  ...
  void _drawABOccupations(Canvas canvas) {
    ...
    if (controller.controlTableModeEnabled) {
      for (var abConfig in controller.controlTableConfig.abConfigurations.values) {
        ...
        final midX = (ac1.position.dx + ac2.position.dx) / 2;
        final midY = (ac1.position.dy + ac2.position.dy) / 2;
        ...
        if (isOccupied) {
          ...
          canvas.drawLine(
            Offset(ac1.position.dx, ac1.position.dy),
            Offset(ac2.position.dx, ac2.position.dy),
            linePaint,
          );
  ...
  void _drawRouteReservations(Canvas canvas) {
    for (var reservation in controller.routeReservations.values) {
      ...
      if (signal == null || !signal.commissioned) continue;

      ControlTableEntry? activeEntry;
      final activeRouteId = signal.activeRouteId;
      if (activeRouteId != null) {
        for (final entry
            in controller.controlTableConfig.getEntriesForSignal(signal.id)) {
          if (entry.routeId == activeRouteId) {
            activeEntry = entry;
            break;
          }
        }
      ...
      if (shouldShowReservation) {
        ...
        for (var blockId in reservation.reservedBlocks) {
          ...
          if (blockId.startsWith('crossover')) {
            final drawn = _drawCrossoverReservationForEntry(
                canvas, block, reservationColor, activeEntry);
            if (!drawn) {
              _drawCrossoverReservation(canvas, block, reservationColor);
            }
          } else {
  ...
  void _drawReservationPreview(Canvas canvas) {
    if (!controller.controlTableModeEnabled) return;
    final signalId = controller.reservationPreviewSignalId;
    if (signalId == null) return;

    final entries = controller.controlTableConfig.getEntriesForSignal(signalId);
    if (entries.isEmpty) return;

    final routeId = controller.reservationPreviewRouteId;
    final filteredEntries = routeId == null
        ? entries
        : entries.where((entry) => entry.routeId == routeId).toList();
    if (filteredEntries.isEmpty) return;

    String? firstBlockId;
    final previewColor = Colors.yellow.withOpacity(0.7);
    for (final entry in filteredEntries) {
      final previewBlocks = entry.protectedBlocks.isNotEmpty
          ? entry.protectedBlocks
          : [...entry.requiredBlocksClear, ...entry.approachBlocks];
      if (previewBlocks.isEmpty) continue;

      for (final blockId in previewBlocks) {
        final block = controller.blocks[blockId];
        if (block == null) continue;
        firstBlockId ??= blockId;
        if (blockId.startsWith('crossover')) {
          final drawn = _drawCrossoverReservationForEntry(
              canvas, block, previewColor, entry);
          if (!drawn) {
            _drawCrossoverReservation(canvas, block, previewColor);
          }
        } else {
          _drawBlockReservation(canvas, block, previewColor);
        }
      }
    }

    if (firstBlockId == null) return;
    final firstBlock = controller.blocks[firstBlockId!];
    if (firstBlock != null) {
      final previewLabel =
          routeId == null ? '$signalId preview' : '$signalId ${routeId} preview';
      final textPainter = TextPainter(
        text: TextSpan(
          text: previewLabel,
          style: TextStyle(
      ...
      textPainter.layout();
      textPainter.paint(
          canvas, Offset(firstBlock.startX + 5, firstBlock.y - 30));
    }
  }

  bool _drawCrossoverReservationForEntry(Canvas canvas, BlockSection block,
      Color reservationColor, ControlTableEntry? entry) {
    final offsets = _getReservationDiagonalOffsets(entry, block.id);
    if (offsets == null) return false;
    _drawDiagonalReservation(canvas, offsets[0], offsets[1], reservationColor);
    return true;
  }

  List<Offset>? _getReservationDiagonalOffsets(
      ControlTableEntry? entry, String blockId) {
    if (entry == null) return null;

    final config = _getReservationDiagonalConfig(entry, blockId);
    if (config != null) {
      final fromId = config['fromPoint'] as String?;
      final toId = config['toPoint'] as String?;
      if (fromId != null && toId != null) {
        final from = _getPointOffset(fromId);
        final to = _getPointOffset(toId);
    ...
    if (entry.requiredPointPositions.length < 2) return null;
    final candidates = entry.requiredPointPositions.entries
        .where((entry) => controller.points.containsKey(entry.key))
        .toList();
    if (candidates.length < 2) return null;

    final reverseCandidates = candidates
        .where((entry) => entry.value == PointPosition.reverse)
        .toList();
    final selected = reverseCandidates.length >= 2 ? reverseCandidates : candidates;
    selected.sort((a, b) =>
        controller.points[a.key]!.y.compareTo(controller.points[b.key]!.y));

    final startPoint = controller.points[selected.first.key];
    final endPoint = controller.points[selected.last.key];
    if (startPoint == null || endPoint == null) return null;
    return [Offset(startPoint.x, startPoint.y), Offset(endPoint.x, endPoint.y)];
  }

  Map<String, dynamic>? _getReservationDiagonalConfig(
      ControlTableEntry entry, String blockId) {
    final customConditions = entry.customConditions;
    final diagonalMap = customConditions['reservationDiagonals'];
    if (diagonalMap is Map && diagonalMap[blockId] is Map) {
      return Map<String, dynamic>.from(diagonalMap[blockId] as Map);
    }

    final legacyConfig = customConditions['reservationDiagonal'];
    if (legacyConfig is Map) {
      return Map<String, dynamic>.from(legacyConfig);
    }

    return null;
  }

  Offset? _getPointOffset(String pointId) {
    final point = controller.points[pointId];
    if (point == null) return null;
    return Offset(point.x, point.y);
  }

  void _drawDiagonalReservation(
      Canvas canvas, Offset start, Offset end, Color reservationColor) {
    ...
    const gapLength = 5.0;
    const reservationOffset = 3.0;

    final delta = end - start;
    final totalDistance = delta.distance;
    if (totalDistance <= 0.1) return;

    final dirX = delta.dx / totalDistance;
    final dirY = delta.dy / totalDistance;
    final offsetX = -dirY * reservationOffset;
    final offsetY = dirX * reservationOffset;
    ...
    while (currentDistance < totalDistance) {
      ...
      final t2 = math.min((currentDistance + dashLength) / totalDistance, 1.0);

      if (drawDash) {
        final x1 = start.dx + (delta.dx * t1) + offsetX;
        final y1 = start.dy + (delta.dy * t1) + offsetY;
        final x2 = start.dx + (delta.dx * t2) + offsetX;
        final y2 = start.dy + (delta.dy * t2) + offsetY;
  ...
  void _drawSignalHead(Canvas canvas, Signal signal) {
    if (!signal.commissioned) {
      final headPaint = Paint()
        ...
        ..style = PaintingStyle.fill;
      _drawSignalHeadShape(canvas, signal, headPaint);

      final strikePaint = Paint()
        ..color = Colors.grey.withOpacity(0.7)
        ..strokeWidth = 3.5;
      canvas.drawLine(
        Offset(signal.x - 12, signal.y - 52),
        Offset(signal.x + 12, signal.y - 32),
        strikePaint,
      );
      canvas.drawLine(
        Offset(signal.x - 12, signal.y - 32),
    ...
    } else if (signal.aspect == SignalAspect.yellow) {
  ...
  void _drawSignalHeadShape(Canvas canvas, Signal signal, Paint headPaint) {
