  ...
  void _drawMovementAuthorities(Canvas canvas) {
    ...
    for (var train in controller.trains) {
      ...
      if (train.cbtcMode != CbtcMode.auto &&
          ...
          train.cbtcMode != CbtcMode.sb &&
          train.cbtcMode != CbtcMode.ism &&
          train.cbtcMode != CbtcMode.csm) continue;

      // MA1, MA2, MA3 - Draw all three levels with different colors
      // Draw MA3 (red/orange - emergency) first (bottom layer)
      if (train.ma3 != null) {
        _drawSingleMA(canvas, train, train.ma3!, Colors.orange, animationOffset, 'MA3');
      }

      // Draw MA2 (yellow - warning) second (middle layer)
      if (train.ma2 != null) {
        _drawSingleMA(canvas, train, train.ma2!, Colors.yellow, animationOffset, 'MA2');
      }

      // Draw MA1 (green - normal) last (top layer)
      if (train.ma1 != null) {
        _drawSingleMA(canvas, train, train.ma1!, Colors.green, animationOffset, 'MA1');
      }

      // Skip old implementation - now using _drawSingleMA for all MA levels
    }
  }

  // MA1, MA2, MA3 - Helper method to draw a single MA level
  void _drawSingleMA(Canvas canvas, Train train, MovementAuthority ma, Color baseColor, double animationOffset, String label) {
    if (ma.maxDistance <= 0) return;

    final trainX = train.x;
    final trainY = train.y;
    final isEastbound = train.direction > 0;

    // Calculate start and end positions for the MA arrow path
    final startX = isEastbound ? trainX + 35 : trainX - 35; // Start just ahead of train
    final endX = isEastbound
        ? trainX + ma.maxDistance
        : trainX - ma.maxDistance;

    // Y-offset based on MA level (stack them)
    final yOffset = label == 'MA1' ? 0.0 : (label == 'MA2' ? 8.0 : 16.0);
    ...
    final pathPaint = Paint()
      ..shader = gradient.createShader(Rect.fromPoints(
        Offset(startX, trainY - 25 + yOffset),
        Offset(endX, trainY - 25 + yOffset),
      ))
      ..style = PaintingStyle.fill;
    ...
    for (int i = 0; i < arrowCount; i++) {
      ...
      final arrowSize = 6.0;
      final arrowY = trainY - 20 + yOffset;
    ...
    labelPainter.paint(
      canvas,
      Offset(
        startX + (isEastbound ? -30 : 10),
        trainY - 23 + yOffset,
      ),
    );

    // Draw destination marker or obstacle indicator at the end (only for MA1)
    if (ma.limitReason != null && label == 'MA1') {
      final endMarkerPaint = Paint()
        ..color = ma.hasDestination ? Colors.blue : baseColor
        ..style = PaintingStyle.fill;

      // Draw vertical line at the limit
      canvas.drawLine(
        Offset(endX, trainY - 30 + yOffset),
        Offset(endX, trainY - 10 + yOffset),
        Paint()
          ..color = ma.hasDestination ? Colors.blue : baseColor
          ..strokeWidth = 3
          ..strokeCap = StrokeCap.round,
      ...
      final reasonPainter = TextPainter(
        text: TextSpan(
          text: ma.limitReason!.length > 20
              ...
              : ma.limitReason!,
          style: TextStyle(
            color: ma.hasDestination ? Colors.blue[900] : baseColor.withOpacity(0.9),
            fontSize: 8,
            fontWeight: FontWeight.bold,
