...
class _ControlTablePanelState extends State<ControlTablePanel>
  ...
  Widget _buildSignalSection(
    ...
    final isCommissioned = signal.commissioned;
      ...
      child: Column(
        children: [
          ...
          InkWell(
            onTap: () {
              if (isCommissioned) {
                final signalModel = controller.signals[signal.id];
                if (signalModel != null) {
                  controller.requestCanvasCenter(
                      Offset(signalModel.x, signalModel.y));
                }
              }
              setState(() {
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isCommissioned
                          ? _getSignalAspectColor(signal.aspect)
                          : Colors.grey[700],
                  ...
                  IconButton(
                    tooltip: 'Preview reservations',
                    icon: Icon(
                      controller.reservationPreviewSignalId == signal.id
                          ? Icons.visibility
                          : Icons.visibility_outlined,
                      color: controller.reservationPreviewSignalId == signal.id
                          ? Colors.yellow
                          : Colors.grey[400],
                      size: 20,
                    ),
                    onPressed: isCommissioned
                        ? () {
                            final isPreview =
                                controller.reservationPreviewSignalId ==
                                    signal.id;
                            controller.setReservationPreview(
                                isPreview ? null : signal.id);
                          }
                        : null,
                  ),
                  Icon(
  ...
  Widget _buildControlTableEntry(
    ...
    final isCommissioned =
        controller.signals[entry.signalId]?.commissioned ?? true;

    return Opacity(
      opacity: isCommissioned ? 1.0 : 0.5,
      child: Container(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  if (controller.controlTableConfig
                      .getEntriesForSignal(entry.signalId)
                      .length > 1)
                    IconButton(
                      tooltip: 'Preview route reservations',
                      icon: Icon(
                        controller.reservationPreviewSignalId == entry.signalId &&
                                controller.reservationPreviewRouteId == entry.routeId
                            ? Icons.visibility
                            : Icons.visibility_outlined,
                        color:
                            controller.reservationPreviewSignalId == entry.signalId &&
                      ...
                      onPressed: isCommissioned
                          ? () {
                              final isPreview =
                                  controller.reservationPreviewSignalId ==
                                          entry.signalId &&
                                      controller.reservationPreviewRouteId ==
                                          entry.routeId;
                              controller.setReservationPreview(
                                isPreview ? null : entry.signalId,
                                routeId: isPreview ? null : entry.routeId,
                              );
                            }
                          : null,
                    ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Reservation diagonal override for crossovers
          _buildReservationDiagonalSection(controller, entry),
  ...
  Widget _buildReservationDiagonalSection(
    TerminalStationController controller,
    ControlTableEntry entry,
  ) {
    ...
    final crossoverBlocks =
        allBlocks.where((blockId) => blockId.startsWith('crossover')).toList();
    if (crossoverBlocks.isEmpty) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.straighten, color: Colors.amber[300], size: 14),
              const SizedBox(width: 6),
              Text(
                'Reservation Diagonal',
                style: TextStyle(
                  color: Colors.grey[300],
                  fontSize: 11,
                  fontWeight: FontWeight.bold,
          ...
          ...crossoverBlocks.map((blockId) {
            final crossover = controller.crossovers[blockId];
            final pointIds = crossover?.pointIds ?? const <String>[];
            if (pointIds.length < 2) {
              return const SizedBox.shrink();
            }

            final config = _getReservationDiagonalConfig(entry, blockId);
            final fromPointId = config?['fromPoint'] as String?;
            final toPointId = config?['toPoint'] as String?;
            final fromValue =
                pointIds.contains(fromPointId) ? fromPointId : null;
            final toValue = pointIds.contains(toPointId) ? toPointId : null;
                    ...
                    children: [
                      ...
                      DropdownButton<String?>(
                        ...
                        items: [
                          ...
                          ...pointIds.map(
                            (pointId) => DropdownMenuItem<String?>(
                              value: pointId,
                              child: Text(pointId),
                            ),
                          ),
  ...
  Map<String, dynamic>? _getReservationDiagonalConfig(
    ...
    final diagonals = customConditions['reservationDiagonals'];
    if (diagonals is Map && diagonals[blockId] is Map) {
      return Map<String, dynamic>.from(diagonals[blockId] as Map);
    }
  ...
  void _updateReservationDiagonal(
    TerminalStationController controller,
    ControlTableEntry entry,
    String blockId, {
    String? fromPointId,
    ...
    final updatedCustom = Map<String, dynamic>.from(entry.customConditions);
    final diagonals =
        Map<String, dynamic>.from(updatedCustom['reservationDiagonals'] as Map? ?? {});
    final current = Map<String, dynamic>.from(diagonals[blockId] as Map? ?? {});

    if (fromPointId == null) {
      current.remove('fromPoint');
    } else {
      current['fromPoint'] = fromPointId;
    }

    if (toPointId == null) {
      current.remove('toPoint');
    } else {
      current['toPoint'] = toPointId;
    }

    if (current.isEmpty) {
      diagonals.remove(blockId);
    } else {
      diagonals[blockId] = current;
    }

    if (diagonals.isEmpty) {
      updatedCustom.remove('reservationDiagonals');
    } else {
      updatedCustom['reservationDiagonals'] = diagonals;
    }

    controller.controlTableConfig.updateEntry(
      entry.copyWith(customConditions: updatedCustom),
    );
    setState(() {});
  }
