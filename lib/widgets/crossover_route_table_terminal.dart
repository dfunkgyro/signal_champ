...
/// Designed for Terminal Station Screen
class CrossoverRouteTableTerminal extends StatefulWidget {
  final TerminalStationController controller;

  const CrossoverRouteTableTerminal({
    Key? key,
    required this.controller,
  }) : super(key: key);

  @override
  State<CrossoverRouteTableTerminal> createState() =>
      _CrossoverRouteTableTerminalState();
}

class _CrossoverRouteTableTerminalState
    extends State<CrossoverRouteTableTerminal> {
  ...
  Widget build(BuildContext context) {
    ...
    String? activeRoute;
    for (final train in widget.controller.trains) {
    ...
    return [
      {
        ...
        'description':
            'Eastbound (Upper Track → Lower Track via 78A/78B reversed)',
      },
      {
        'route': '109→crossover109→crossover106→104',
        'description':
            'Westbound (Lower Track → Upper Track via 78B/78A reversed)',
      },
    ];
  }
}
