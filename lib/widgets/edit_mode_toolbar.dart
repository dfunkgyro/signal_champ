...
class EditModeToolbar extends StatelessWidget {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          // Select Tool
          Tooltip(
            message: 'Select Tool\\nClick to select and move components',
            child: IconButton(
              icon: Icon(
                Icons.touch_app,
                color: controller.currentEditTool == EditTool.select
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () => controller.setEditTool(EditTool.select),
              splashRadius: 20,
            ),
          ),
          ...
          // Add Block Tool
          Tooltip(
            message: 'Add Block Tool\\nClick canvas to add new track block',
            child: IconButton(
              icon: Icon(
                Icons.line_axis,
                color: controller.currentEditTool == EditTool.addBlock
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.addBlock);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click on canvas to place new block'),
          ...
          // Add Signal Tool
          Tooltip(
            message: 'Add Signal Tool\\nClick canvas to add new signal',
            child: IconButton(
              icon: Icon(
                Icons.traffic,
                color: controller.currentEditTool == EditTool.addSignal
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.addSignal);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click on canvas to place new signal'),
                    duration: Duration(seconds: 2),
                    backgroundColor: Colors.blue,
                  ),
                );
          ...
          // Add Point Tool
          Tooltip(
            message: 'Add Point Tool\\nClick canvas to add new point/switch',
            child: IconButton(
              icon: Icon(
                Icons.call_split,
                color: controller.currentEditTool == EditTool.addPoint
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.addPoint);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click on canvas to place new point'),
                    duration: Duration(seconds: 2),
                    backgroundColor: Colors.blue,
                  ),
                );
          ...
          // Add Crossover Tool
          Tooltip(
            message: 'Add Crossover Tool\\nDrag on canvas to create crossover',
            child: IconButton(
              icon: Icon(
                Icons.compare_arrows,
                color: controller.currentEditTool == EditTool.addCrossover
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.addCrossover);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click and drag to create crossover'),
                    duration: Duration(seconds: 2),
                    backgroundColor: Colors.blue,
                  ),
                );
          ...
          // Connect Tool
          Tooltip(
            message: 'Connect Tool\\nClick two blocks to connect them',
            child: IconButton(
              icon: Icon(
                Icons.link,
                color: controller.currentEditTool == EditTool.connect
                    ? Colors.yellowAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.connect);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click first block, then second block to connect'),
                    duration: Duration(seconds: 2),
                    backgroundColor: Colors.blue,
                  ),
                );
          ...
          // Delete Tool
          Tooltip(
            message: 'Delete Tool\\nClick components to delete them',
            child: IconButton(
              icon: Icon(
                Icons.delete_sweep,
                color: controller.currentEditTool == EditTool.delete
                    ? Colors.redAccent
                    : Colors.white,
              ),
              onPressed: () {
                controller.setEditTool(EditTool.delete);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Click components to delete them'),
                    duration: Duration(seconds: 2),
          ...
          Tooltip(
            message: 'Validate Network\\nCheck for errors and issues',
            child: IconButton(
              icon: const Icon(Icons.check_circle_outline, color: Colors.greenAccent),
              onPressed: () => _showValidationResults(context, controller),
              splashRadius: 20,
            ),
          ),
          ...
          Tooltip(
            ...
            child: IconButton(
              ...
              onPressed: () => _showNetworkStats(context, controller),
              splashRadius: 20,
            ),
          ),
  ...
  /// Show network validation results dialog
  void _showValidationResults(BuildContext context, TerminalStationController controller) {
    final issues = controller.editorValidateNetwork();
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Text(issues.isEmpty ? 'Network Valid' : 'Validation Issues'),
          ],
        ),
        content: SizedBox(
          width: 500,
          child: issues.isEmpty
              ? const Text('✅ No issues found. Network is valid.')
              : Column(
                  mainAxisSize: MainAxisSize.min,
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                          ...
                          return Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Expanded(child: Text(issues[index])),
                              ],
                            ),
                          );
                        },
  ...
  void _showNetworkStats(BuildContext context, TerminalStationController controller) {
    final stats = controller.editorGetNetworkStats();
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildStatRow('Total Blocks:', '${stats['totalBlocks']}'),
              _buildStatRow('Total Signals:', '${stats['totalSignals']}'),
              _buildStatRow('Total Points:', '${stats['totalPoints']}'),
              _buildStatRow('Total Length:', '${stats['totalLength']?.toStringAsFixed(0) ?? 0}m'),
              const Divider(height: 24),
              _buildStatRow('Crossovers:', '${stats['crossovers']}'),
              _buildStatRow('Main Line Blocks:', '${stats['mainLineBlocks']}'),
              _buildStatRow('Electrified Blocks:', '${stats['electrifiedBlocks']}'),
            ],
          ),
        ),
        actions: [
