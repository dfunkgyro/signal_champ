...
class _MaintenancePropertyEditorState extends State<MaintenancePropertyEditor> {
  ...
  Widget build(BuildContext context) {
    ...
    if (editController.primarySelection == null &&
        stationController.selectedComponentType != null &&
        stationController.selectedComponentId != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        editController.selectComponent(
          stationController.selectedComponentType!,
          stationController.selectedComponentId!,
        );
      });
    }
  ...
  Widget _buildSignalProperties(
    ...
    return ListView(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: _buildNumberField(
                controllerKey: 'signal_${signal.id}_x',
                label: 'X',
                value: signal.x,
                onChanged: (value) {
                  if (value != null) {
            ...
            Expanded(
              child: _buildNumberField(
                controllerKey: 'signal_${signal.id}_y',
                label: 'Y',
                value: signal.y,
                onChanged: (value) {
                  if (value != null) {
        ...
        _buildPropertyLabel('Commissioned'),
        Switch(
          value: signal.commissioned,
          onChanged: (value) {
            editController.setSignalCommissioned(signal.id, value);
          },
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: _buildNumberField(
                controllerKey: 'point_${point.id}_x',
                label: 'X',
                value: point.x,
                onChanged: (value) {
                  if (value != null) {
            ...
            Expanded(
              child: _buildNumberField(
                controllerKey: 'point_${point.id}_y',
                label: 'Y',
                value: point.y,
                onChanged: (value) {
                  if (value != null) {
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: _buildNumberField(
                controllerKey: 'block_${block.id}_x',
                label: 'X',
                value: block.x,
                onChanged: (value) {
                  if (value != null) {
            ...
            Expanded(
              child: _buildNumberField(
                controllerKey: 'block_${block.id}_y',
                label: 'Y',
                value: block.y,
                onChanged: (value) {
                  if (value != null) {
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: _buildNumberField(
                controllerKey: 'axleCounter_${ac.id}_x',
                label: 'X',
                value: ac.x,
                onChanged: (value) {
                  if (value != null) {
            ...
            Expanded(
              child: _buildNumberField(
                controllerKey: 'axleCounter_${ac.id}_y',
                label: 'Y',
                value: ac.y,
                onChanged: (value) {
                  if (value != null) {
  ...
  Widget _buildNumberField({
    ...
    final controller = _getController(controllerKey, value.toStringAsFixed(1));
  ...
  Widget _buildActionButtons(MaintenanceEditController editController) {
    final selection = editController.primarySelection;
    final hasSingleSelection = editController.selectionCount == 1;
    return Column(
      children: [
          ...
          children: [
            Expanded(
              child: ElevatedButton.icon(
                onPressed: selection != null && hasSingleSelection
                    ? () => _applyPositionEdits(selection, editController)
                    : null,
  ...
  void _applyPositionEdits(
    SelectedComponentInfo selection,
    MaintenanceEditController editController,
  ) {
    final currentPosition = _getComponentPosition(selection, editController);
    if (currentPosition == null) return;

    final xKey = _positionKey(selection, 'x');
    final yKey = _positionKey(selection, 'y');
    final xText = _controllers[xKey]?.text ?? '';
    final yText = _controllers[yKey]?.text ?? '';
    final newX = double.tryParse(xText) ?? currentPosition.dx;
    final newY = double.tryParse(yText) ?? currentPosition.dy;

    if (newX == currentPosition.dx && newY == currentPosition.dy) return;
  ...
  Offset? _getComponentPosition(
    SelectedComponentInfo selection,
    MaintenanceEditController editController,
  ) {
    final controller = editController.stationController;
    switch (selection.type) {
      case 'signal':
        final signal = controller.signals[selection.id];
        return signal == null ? null : Offset(signal.x, signal.y);
      case 'point':
        final point = controller.points[selection.id];
        return point == null ? null : Offset(point.x, point.y);
      case 'block':
        final block = controller.blockSections[selection.id];
        return block == null ? null : Offset(block.x, block.y);
      case 'axleCounter':
        final ac = controller.axleCounters[selection.id];
        return ac == null ? null : Offset(ac.x, ac.y);
    }
    return null;
  }

  String _positionKey(SelectedComponentInfo selection, String axis) {
    return '${selection.type}_${selection.id}_$axis';
  }
