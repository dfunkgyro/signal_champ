...
void main() {
  ...
  group('TimetableManager Model Tests', () {
    late TimetableManager manager;

    setUp(() {
      manager = TimetableManager();
    });

    test('TimetableManager should start with empty timetables', () {
      expect(manager.timetables.length, 0);
      expect(manager.ghostTrains.length, 0);
    });

    test('TimetableManager should add timetable entries', () {
      final entry = TimetableEntry(
        id: 'TT001',
        trainServiceNumber: '101',
        stops: [
      ...
      manager.addTimetable(entry);
      expect(manager.timetables.length, 1);
      expect(manager.timetables[0].id, 'TT001');
    });

    test('TimetableManager should create ghost trains from timetables', () {
      final entry = TimetableEntry(
        id: 'TT001',
        trainServiceNumber: '101',
        stops: [
      ...
      manager.addTimetable(entry);
      final ghostTrain = manager.createGhostTrain('TT001');

      expect(ghostTrain, isNotNull);
      expect(ghostTrain!.timetableId, 'TT001');
      expect(ghostTrain.serviceNumber, '101');
      expect(manager.ghostTrains.length, 1);
    });

    test('TimetableManager should assign ghost train to real train', () {
      final entry = TimetableEntry(
        id: 'TT001',
        trainServiceNumber: '101',
        stops: [
      ...
      manager.addTimetable(entry);
      final ghostTrain = manager.createGhostTrain('TT001');
      manager.assignGhostTrainToReal(ghostTrain!.id, 'TRAIN_001');

      final assigned = manager.getGhostTrain(ghostTrain.id);
      expect(assigned?.assignedToRealTrain, true);
      expect(assigned?.realTrainId, 'TRAIN_001');
    });

    test('TimetableManager should get timetable by ID', () {
      final entry = TimetableEntry(
        id: 'TT001',
        trainServiceNumber: '101',
        stops: [
      ...
      manager.addTimetable(entry);
      final retrieved = manager.getTimetable('TT001');

      expect(retrieved, isNotNull);
      expect(retrieved!.id, 'TT001');
    });

    test('TimetableManager should return null for non-existent timetable', () {
      final retrieved = manager.getTimetable('INVALID');
      expect(retrieved, null);
    });

    test('TimetableManager should get ghost trains by real train ID', () {
      final entry = TimetableEntry(
        id: 'TT001',
        trainServiceNumber: '101',
        stops: [
      ...
      final found = manager.getGhostTrainByRealTrain('TRAIN_001');
      expect(found, isNotNull);
      expect(found!.realTrainId, 'TRAIN_001');
    });
  });
}
